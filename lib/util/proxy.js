...
var genObjectProxy = function(serviceName, origin, attach, proxyCB) {
  ...
  if(!!origin.constructor){
    const properties = Object.getOwnPropertyNames(origin.constructor.prototype)
    for (var index in properties){
      const field = properties[index]
      if(typeof origin[field] === 'function' && !res[field]) {
        res[field] = genFunctionProxy(serviceName, field, origin, attach, proxyCB);
      } 
    }
  }
...
var genFunctionProxy = function(serviceName, methodName, origin, attach, proxyCB) {
  return (function() {
    ...
    if(attach.namespace === 'user'){
      return Promise.promisify(proxy)
    }
